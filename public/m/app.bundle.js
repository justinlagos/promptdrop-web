...
const PDsvc = {
  ...
  async ask(question, context, mode) {
    try {
      const r = await fetch(PD_FN + "/assistant", { method: "POST", headers: { "Content-Type": "application/json", Authorization: "Bearer " + PD_SUPA_ANON, apikey: PD_SUPA_ANON }, body: JSON.stringify({ question, context, mode: mode || "task" }) });
      const d = await r.json().catch(() => ({}));
      if (r.status === 503) return { ok: false, msg: "AI is not switched on yet on the server." };
...
function LiveSession({ cfg, onExit, onComplete }) {
  ...
  async function finishRec() {
    ...
    const save = async (blob) => {
      ...
      try {
        await PDsvc.putTake({ id, createdAt: Date.now(), durationMs: dur, size: blob ? blob.size : 0, type: blob ? blob.type || "" : "", source: "camera", title: cfg.title, blob: blob || null });
      } catch (e) {
      }
      stopStream();
      onComplete({ id });
    ...
    const finalize = () => {
      ...
      save(new Blob(chunksRef.current, { type: mr.mimeType || "video/webm" }));
    };
    mr.onstop = finalize;
    try {
  ...
  React.useEffect(() => {
    ...
    if (count === 0) {
      if (!recording || camOk || camErr) setPhase("prompting");
      return;
    }
  ...
  React.useEffect(() => {
    if (phase === "prompting" && camOk) startRec();
  }, [phase, camOk]);
  React.useEffect(() => {
...
function TakeReviewScreen({ app }) {
  ...
  function download() {
    if (!url) return;
    const ext = take && take.type && take.type.indexOf("mp4") >= 0 ? "mp4" : "webm";
    const a = document.createElement("a");
    a.href = url;
    a.download = "promptdrop-take." + ext;
    a.click();
  }
  return /* @__PURE__ */ React.createElement(Screen, null, /* @__PURE__ */ React.createElement(NavHeader, { onBack: app.back, title: "Take", action: hasVideo ? /* @__PURE__ */ React.createElement("button", { className: "pd-tap", style: iconBtn, onClick: download }, /* @__PURE__ */ React.createElement(Icon, { name: "download", size: 19, color: "var(--text-secondary)" })) : null }), /* @__PURE__ */ React.createElement(ScrollArea, { padBottom: 120 }, /* @__PURE__ */ React.createElement("div", { style: { margin: "6px 16px 0", borderRadius: 22, overflow: "hidden", position: "relative", aspectRatio: "9 / 13", maxHeight: 380, background: "#000" } }, hasVideo ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("video", { ref: vref, src: url, playsInline: true, controls: true, preload: "metadata", onPlay: () => setPlaying(true), onPause: () => setPlaying(false), onEnded: () => setPlaying(false), style: { width: "100%", height: "100%", objectFit: "cover", background: "#000" } }), !playing && /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" } }, /* @__PURE__ */ React.createElement("button", { className: "pd-tap", onClick: toggle, style: { width: 64, height: 64, borderRadius: 999, border: "none", background: "rgba(255,255,255,0.92)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", pointerEvents: "auto" } }, /* @__PURE__ */ React.createElement(Icon, { name: "play", size: 26, color: "#000" })))) : /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 26, textAlign: "center", gap: 8 } }, /* @__PURE__ */ React.createElement(PromptCharacter, { state: loaded ? "idle" : "processing", size: 64 }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 15, fontWeight: 600, color: "var(--text-primary)" } }, loaded ? "No video was captured" : "Loading your take\u2026"), loaded && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "var(--text-muted)", lineHeight: 1.45, maxWidth: 260 } }, "This browser may not support in-app video recording. Allow camera and microphone access, or record from the desktop app."))), /* @__PURE__ */ React.createElement("div", { style: { padding: "16px 20px 0" } }, /* @__PURE__ */ React.createElement(Eyebrow, null, take ? pd.svc.rel(take.createdAt) : ""), /* @__PURE__ */ React.createElement("h2", { style: { margin: "6px 0 0", fontSize: 22, fontWeight: 800, letterSpacing: "-0.02em", color: "var(--text-primary)" } }, take ? take.title || "Take" : "Loading\u2026")), /* @__PURE__ */ React.createElement("div", { style: { padding: "14px 20px 0" } }, /* @__PURE__ */ React.createElement("div", { style: { background: "var(--surface-elevated)", borderRadius: 16, overflow: "hidden" } }, insights.map((i, n) => /* @__PURE__ */ React.createElement("div", { key: i.label, style: { display: "flex", alignItems: "center", gap: 13, padding: "14px 16px", borderBottom: n === insights.length - 1 ? "none" : "1px solid var(--border-subtle)" } }, /* @__PURE__ */ React.createElement(Icon, { name: i.icon, size: 19, color: "var(--text-muted)", style: { flexShrink: 0 } }), /* @__PURE__ */ React.createElement("span", { style: { flex: 1, fontSize: 14.5, color: "var(--text-secondary)" } }, i.label), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14.5, fontWeight: 600, color: "var(--text-primary)" } }, i.value)))))), /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", left: 0, right: 0, bottom: 0, padding: "12px 20px calc(12px + env(safe-area-inset-bottom, 22px))", background: "linear-gradient(transparent, var(--bg-primary) 28%)", display: "flex", gap: 10 } }, /* @__PURE__ */ React.createElement(DSr.Button, { variant: "secondary", size: "lg", onClick: async () => {
    if (take) {
...
function ctMerge(segs) {
  ...
  for (const seg of s) {
    ...
    if (last && seg.start <= last.end + 1e-3) last.end = Math.max(last.end, seg.end);
    else out.push({ ...seg });
  }
  return out;
}
function ctKept(duration, removed) {
  const rm = ctMerge(removed);
  const kept = [];
  let cur = 0;
  for (const r of rm) {
    if (r.start > cur) kept.push({ start: cur, end: Math.min(r.start, duration) });
...
function ctStamp(t, comma) {
  t = Math.max(0, t || 0);
  const h = Math.floor(t / 3600), m = Math.floor(t % 3600 / 60), s = Math.floor(t % 60), ms = Math.floor((t - Math.floor(t)) * 1e3);
...
function ctDownloadText(text, name) {
  const a = document.createElement("a");
...
async function ctDetectSilences(blob) {
  try {
    ...
    const floor = Math.max(median * 1.8, peak * 0.06, 4e-3);
    const sil = [];
    let runStart = -1;
...
function ctPauseDecisions(silences, preset) {
  const cfg = CT_PRESETS[preset];
  if (!cfg || !silences) return [];
  const out = [];
  for (const s of silences) {
    const len = s.end - s.start;
    if (len < cfg.min) continue;
    const start = s.start + cfg.keep / 2, end = s.end - cfg.keep / 2;
    if (end - start > 0.12) out.push({ id: "p" + Math.round(s.start * 1e3), type: "pause", start, end, active: true, source: "auto", label: "Pause " + len.toFixed(1) + "s" });
  }
  return out;
}
function CleanPreview({ url, removed, rate, filterCss, active, trimStart, trimEnd, duration, vref }) {
  ...
  const applyRate = React.useCallback(() => {
    ...
    if (v) {
      try {
        v.playbackRate = active ? rate || 1 : 1;
      } catch (e) {
      }
    }
  }, [rate, active]);
  ...
  function onTime() {
    ...
    if (trimEnd > 0 && duration && t >= duration - trimEnd - 0.05) {
      try {
        v.pause();
      } catch (e) {
      }
  ...
  function onPlay() {
    applyRate();
    const v = ref.current;
    if (v && active && trimStart > 0 && v.currentTime < trimStart) {
      try {
        v.currentTime = trimStart + 1e-3;
      } catch (e) {
      }
    }
  }
  return /* @__PURE__ */ React.createElement("video", { ref, src: url, playsInline: true, controls: true, preload: "metadata", onLoadedMetadata: applyRate, onPlay, onRateChange: applyRate, onTimeUpdate: onTime, style: { width: "100%", height: "100%", objectFit: "contain", background: "#000", filter: filterCss || "none", transition: "filter .12s ease" } });
}
function CleanTakeScreen({ app }) {
  ...
  const [url, setUrl] = React.useState(null);
  const urlRef = React.useRef(null);
  const [tab, setTab] = React.useState("Clean");
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      if (t && t.blob && t.blob.size > 0) {
        ...
        setUrl(u);
        if (t.clean) {
          setPreset(t.clean.preset || "Balanced");
          setSilences(t.clean.silences || null);
          setDuration(t.clean.duration || t.durationMs / 1e3);
          setAnalysing(false);
        } else {
          ...
          if (!live) return;
          if (res) {
            setSilences(res.silences);
            setDuration(res.duration || t.durationMs / 1e3);
          } else {
            setDuration(t.durationMs / 1e3);
          }
  ...
  const hasVideo = !!url;
  const dur = duration || (take ? take.durationMs / 1e3 : 0);
  ...
  async function save() {
    ...
    try {
      await pd.svc.putTake({ ...take, clean });
    } catch (e) {
    }
  ...
  async function transcribeTake() {
    ...
    if (tr.ok) {
      const updated = { ...take, transcript: tr.text, segments: tr.segments || [] };
      await pd.svc.putTake(updated);
      setTake(updated);
      app.toast("Transcribed");
  ...
  function setStartFromPlayhead() {
    const t = Math.floor(vref.current && vref.current.currentTime || 0);
    setTrimStart(Math.max(0, Math.min(t, Math.floor(dur / 2))));
  }
  function setEndFromPlayhead() {
    const t = vref.current && vref.current.currentTime || 0;
    setTrimEnd(Math.max(0, Math.min(Math.floor(dur - t), Math.floor(dur / 2))));
  }
...
function CtCard({ title, hint, children }) {
  return /* @__PURE__ */ React.createElement("div", { style: { background: "var(--surface-elevated)", border: "1px solid var(--border-subtle)", borderRadius: 16, padding: 16 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14.5, fontWeight: 700, color: "var(--text-primary)" } }, title), hint && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 2, marginBottom: 12, fontSize: 12, color: "var(--text-muted)", lineHeight: 1.4 } }, hint), !hint && /* @__PURE__ */ React.createElement("div", { style: { height: 12 } }), children);
}
...
function CtMoreSheet({ onClose, onReset, onDownload, onDelete }) {
  const row = (label, onClick, danger) => /* @__PURE__ */ React.createElement("button", { className: "pd-tap", onClick, style: { width: "100%", textAlign: "left", height: 54, padding: "0 16px", borderRadius: 14, border: "none", background: "var(--surface-elevated)", color: danger ? "var(--recording)" : "var(--text-primary)", fontSize: 15.5, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 12 } }, /* @__PURE__ */ React.createElement(Icon, { name: danger ? "trash" : label === "Reset Clean Take" ? "redo" : "download", size: 18, color: danger ? "var(--recording)" : "var(--text-secondary)" }), label);
  return /* @__PURE__ */ React.createElement("div", { onClick: onClose, style: { position: "absolute", inset: 0, zIndex: 55, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "flex-end" } }, /* @__PURE__ */ React.createElement("div", { onClick: (e) => e.stopPropagation(), className: "pd-push", style: { width: "100%", background: "var(--bg-secondary)", borderTopLeftRadius: 22, borderTopRightRadius: 22, border: "1px solid var(--border-default)", borderBottom: "none", padding: "14px 16px calc(18px + env(safe-area-inset-bottom, 22px))" } }, /* @__PURE__ */ React.createElement("div", { style: { width: 40, height: 5, borderRadius: 9, background: "var(--ink-600)", margin: "0 auto 16px" } }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 8 } }, onDownload && row("Download original", onDownload), row("Reset Clean Take", onReset), row("Delete take", onDelete, true)), /* @__PURE__ */ React.createElement("button", { className: "pd-tap", onClick: onClose, style: { marginTop: 14, width: "100%", height: 50, borderRadius: 14, border: "1px solid var(--border-default)", background: "transparent", color: "var(--text-secondary)", fontSize: 15, fontWeight: 600, cursor: "pointer" } }, "Cancel")));
}
function Field({ label, hint, children }) {
  return /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 8 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14, fontWeight: 600, color: "var(--text-primary)" } }, label), hint && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11.5, color: "var(--text-muted)" } }, hint)), children);
}
function ExportRow({ label, sub, ready, pending, note, onClick }) {
  return /* @__PURE__ */ React.createElement("button", { className: "pd-tap", onClick: ready ? onClick : void 0, style: { width: "100%", textAlign: "left", background: "var(--surface-elevated)", border: "1px solid var(--border-default)", borderRadius: 14, padding: 14, display: "flex", alignItems: "center", gap: 12, cursor: ready ? "pointer" : "default", opacity: pending ? 0.7 : 1 } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 15, fontWeight: 600, color: "var(--text-primary)" } }, label), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 2, fontSize: 12.5, color: "var(--text-muted)" } }, note || sub)), ready ? /* @__PURE__ */ React.createElement(Icon, { name: "download", size: 18, color: "var(--accent-primary)" }) : /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--text-muted)", letterSpacing: ".06em" } }, "SOON"));
}
function EditMapSheet({ dur, removed, restored, onToggle, pauses, onClose }) {
  const segs = ctMerge(removed);
  return /* @__PURE__ */ React.createElement("div", { onClick: onClose, style: { position: "absolute", inset: 0, zIndex: 50, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "flex-end" } }, /* @__PURE__ */ React.createElement("div", { onClick: (e) => e.stopPropagation(), className: "pd-push", style: { width: "100%", background: "var(--bg-secondary)", borderTopLeftRadius: 22, borderTopRightRadius: 22, border: "1px solid var(--border-default)", borderBottom: "none", padding: "14px 20px calc(20px + env(safe-area-inset-bottom, 22px))", maxHeight: "70%", overflowY: "auto" } }, /* @__PURE__ */ React.createElement("div", { style: { width: 40, height: 5, borderRadius: 9, background: "var(--ink-600)", margin: "0 auto 14px" } }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 17, fontWeight: 800, color: "var(--text-primary)", marginBottom: 4 } }, "Edit Map"), /* @__PURE__ */ React.createElement("p", { style: { fontSize: 12.5, color: "var(--text-muted)", margin: "0 0 14px" } }, "Green is kept, red is removed. Tap a removed part to restore it."), /* @__PURE__ */ React.createElement("div", { style: { position: "relative", height: 26, borderRadius: 8, overflow: "hidden", background: "var(--success, #34C759)", marginBottom: 16 } }, segs.map((s, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { position: "absolute", top: 0, bottom: 0, left: s.start / dur * 100 + "%", width: Math.max(0.6, (s.end - s.start) / dur * 100) + "%", background: "var(--recording, #FF3B30)" } }))), pauses.length === 0 && /* @__PURE__ */ React.createElement("p", { style: { fontSize: 13.5, color: "var(--text-muted)" } }, "No pauses detected to remove. Trims still apply."), pauses.map((p) => {
    const kept = restored[p.id];
    return /* @__PURE__ */ React.createElement("div", { key: p.id, style: { display: "flex", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: "1px solid var(--border-subtle)" } }, /* @__PURE__ */ React.createElement("span", { style: { width: 9, height: 9, borderRadius: 999, background: kept ? "var(--success, #34C759)" : "var(--recording, #FF3B30)", flexShrink: 0 } }), /* @__PURE__ */ React.createElement("span", { style: { flex: 1, fontSize: 14.5, color: "var(--text-primary)" } }, p.label, " ", /* @__PURE__ */ React.createElement("span", { style: { color: "var(--text-muted)", fontSize: 12 } }, "at ", ctClock(p.start))), /* @__PURE__ */ React.createElement("button", { className: "pd-tap", onClick: () => onToggle(p.id), style: { height: 32, padding: "0 12px", borderRadius: 999, border: "1px solid var(--border-default)", background: kept ? "var(--surface-raised)" : "var(--surface-elevated)", color: "var(--text-primary)", fontSize: 12.5, fontWeight: 600, cursor: "pointer" } }, kept ? "Removed" : "Restore"));
  }), /* @__PURE__ */ React.createElement("button", { className: "pd-tap", onClick: onClose, style: { marginTop: 16, width: "100%", height: 46, borderRadius: 14, border: "none", background: "var(--accent-primary)", color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer" } }, "Done")));
}
...
function MeetingLive({ onExit, onDone }) {
  ...
  function pickMime() {
    const k = ["audio/webm;codecs=opus", "audio/webm", "audio/mp4"];
    if (!window.MediaRecorder) return null;
    for (const m of k) {
      try {
        if (MediaRecorder.isTypeSupported(m)) return m;
          ...
          try {
            ...
            r.onresult = (ev) => {
              ...
              for (let i = ev.resultIndex; i < ev.results.length; i++) {
                ...
                if (ev.results[i].isFinal) fin += seg + " ";
                else itr += seg;
              }
              if (fin) {
                liveRef.current = (liveRef.current + " " + fin).trim();
  ...
  async function doAsk(q) {
    ...
    setAsking(true);
    setAnswer(null);
    const ctx = (liveRef.current || "").trim();
    const r = await PDsvc.ask(q, ctx || "(nothing captured yet)", "ask");
    setAsking(false);
    setAnswer(r.ok ? r.text : r.msg || "Could not answer.");
    setAsk("");
  }
  async function stop() {
    const mr = recRef.current;
    ...
    const finish = async (blob) => {
      ...
      try {
        await PDsvc.putTake({ id, createdAt: Date.now(), durationMs: dur, size: blob ? blob.size : 0, source: "meeting", title, blob, transcript, segments, summary, actions, decisions, questions });
      } catch (e) {
      }
      onDone(id);
    };
  ...
  return /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", inset: 0, background: "var(--ink-1000, #050506)", zIndex: 30, display: "flex", flexDirection: "column" } }, /* @__PURE__ */ React.createElement("div", { style: { paddingTop: TOP_INSET, paddingLeft: 16, paddingRight: 16, paddingBottom: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 10, background: "var(--surface-elevated)", border: "1px solid var(--border-default)", borderRadius: 12, padding: "10px 13px" } }, /* @__PURE__ */ React.createElement(Icon, { name: "shield", size: 18, color: "var(--accent-primary)" }), /* @__PURE__ */ React.createElement("span", { style: { flex: 1, fontSize: 12.5, color: "var(--text-secondary)", lineHeight: 1.35 } }, "Recording. You are responsible for getting everyone's consent first."))), phase === "recording" && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", padding: "2px 0" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "inline-flex", alignItems: "center", gap: 8 } }, /* @__PURE__ */ React.createElement("span", { className: "pd-rec-dot" }), /* @__PURE__ */ React.createElement("span", { style: { fontFamily: "var(--font-mono)", fontSize: 26, fontWeight: 500, color: "var(--text-primary)" } }, fmt(elapsed))), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 2, fontFamily: "var(--font-mono)", fontSize: 10, letterSpacing: "0.14em", color: "#FF8077" } }, "RECORDING")), /* @__PURE__ */ React.createElement("div", { ref: scrollRef, className: "pd-scroll", style: { flex: 1, minHeight: 0, overflowY: "auto", padding: "8px 20px 0" } }, /* @__PURE__ */ React.createElement(Eyebrow, { style: { marginBottom: 10 } }, "Live transcript"), live || interim ? /* @__PURE__ */ React.createElement("p", { style: { fontSize: 16, color: "var(--text-primary)", lineHeight: 1.5 } }, live, " ", /* @__PURE__ */ React.createElement("span", { style: { color: "var(--text-muted)" } }, interim)) : /* @__PURE__ */ React.createElement("p", { style: { fontSize: 14, color: "var(--text-muted)", lineHeight: 1.5 } }, captionsOn ? "Listening\u2026" : "Live captions are not supported in this browser. The full transcript is ready when you stop.")), answer && /* @__PURE__ */ React.createElement("div", { style: { margin: "0 16px 8px", padding: "12px 14px", background: "var(--surface-elevated)", border: "1px solid var(--border-default)", borderRadius: 14, fontSize: 14, color: "var(--text-primary)", lineHeight: 1.5, whiteSpace: "pre-wrap" } }, answer), /* @__PURE__ */ React.createElement("div", { style: { padding: "4px 16px 0", display: "flex", gap: 9, alignItems: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 1, display: "flex", alignItems: "center", background: "var(--surface-elevated)", border: "1px solid var(--border-default)", borderRadius: 999, padding: "0 6px 0 14px", height: 44 } }, /* @__PURE__ */ React.createElement("input", { value: ask, onChange: (e) => setAsk(e.target.value), onKeyDown: (e) => e.key === "Enter" && doAsk(ask), placeholder: "Ask mid-call, e.g. what should I say?", style: { flex: 1, background: "none", border: "none", outline: "none", color: "var(--text-primary)", fontSize: 14, fontFamily: "var(--font-sans)" } }), /* @__PURE__ */ React.createElement("button", { className: "pd-tap", onClick: () => doAsk(ask), style: { width: 32, height: 32, borderRadius: 999, border: "none", background: ask.trim() ? "var(--accent-primary)" : "var(--surface-raised)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 } }, /* @__PURE__ */ React.createElement(Icon, { name: "send", size: 15, color: ask.trim() ? "#fff" : "var(--text-muted)" })))), asking && /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", fontSize: 12, color: "var(--text-muted)", padding: "6px 0 0" } }, "Thinking\u2026"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "center", gap: 40, padding: "12px 0 calc(22px + env(safe-area-inset-bottom, 22px))" } }, /* @__PURE__ */ React.createElement(RoundBtn, { size: 52, bg: "var(--surface-elevated)", style: { borderColor: "var(--border-default)" }, onClick: onExit }, /* @__PURE__ */ React.createElement(Icon, { name: "x", size: 20, color: "var(--text-secondary)" })), /* @__PURE__ */ React.createElement("button", { className: "pd-tap", onClick: stop, style: { width: 72, height: 72, borderRadius: 999, border: "4px solid rgba(255,255,255,0.2)", background: "var(--recording)", boxShadow: "var(--glow-recording)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" } }, /* @__PURE__ */ React.createElement("span", { style: { width: 22, height: 22, borderRadius: 6, background: "#fff" } })), /* @__PURE__ */ React.createElement("div", { style: { width: 52 } }))), phase === "processing" && /* @__PURE__ */ React.createElement("div", { style: { flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, paddingBottom: 60 } }, /* @__PURE__ */ React.createElement(PromptCharacter, { state: "processing", size: 92 }), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 18, fontWeight: 700, color: "var(--text-primary)" } }, "Making sense of the call\u2026"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13.5, color: "var(--text-muted)" } }, note)));
}
...
class PDErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.err) return /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", padding: 24, background: "var(--bg-primary, #0a0a0b)", color: "#cfd2da", font: "14px system-ui", textAlign: "center" } }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontWeight: 700, marginBottom: 8 } }, "Something went wrong"), /* @__PURE__ */ React.createElement("div", { style: { opacity: 0.6, fontSize: 12 } }, String(this.state.err && this.state.err.message || this.state.err)), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 14 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => location.reload(), style: { padding: "8px 16px", borderRadius: 10, border: "1px solid #333", background: "#1a1a1f", color: "#fff", fontSize: 13 } }, "Reload"))));
...
if (_mount) {
  ...
  ReactDOM.createRoot(_mount).render(/* @__PURE__ */ React.createElement(DataProvider, null, /* @__PURE__ */ React.createElement(PDErrorBoundary, null, /* @__PURE__ */ React.createElement(PromptDropApp, null))));
}
